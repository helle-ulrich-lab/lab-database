...
class Config:
    ...
    def lookup_port(self, server_index):
        ...
        if self.cfile.has_section(sectionName) == False:
            raise Exception("server index %s is not defined in configuration file" % server_index)

        if self.cfile.has_option(sectionName, "enable") == False or self.cfile.getint(sectionName, "enable") == 0:
            raise Exception("server index %s is disabled" % server_index)

        # get the tcp port
        if (self.cfile.has_option(sectionName, "tcpPort") == False):
    ...
    def get_server_ports(self):
        ...
        for i in range(1,100):
            ...
            if self.cfile.has_section(sectionName) and \
                (self.cfile.has_option(sectionName, "enable") == False or self.cfile.getint(sectionName, "enable") != 0) and \
                self.cfile.has_option(sectionName, "tcpPort"):
