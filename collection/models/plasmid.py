...
class Plasmid(DownloadFileNameMixin, models.Model, SaveWithoutHistoricalRecord):
    ...
    parent_vector = models.ForeignKey(
        ...
        related_name="plasmid_parent_vector",
    ...
    map_png = models.ImageField(
        "plasmid image", upload_to=_model_upload_to + "png/", blank=True
    )
    ...
    approval_user = models.ForeignKey(
        User, related_name="plasmid_approval_user", on_delete=models.PROTECT, null=True
    )
    created_by = models.ForeignKey(
        User, related_name="plasmid_createdby_user", on_delete=models.PROTECT
    )
    ...
    formz_projects = models.ManyToManyField(
        ...
        related_name="plasmid_formz_projects",
        blank=False,
    )
    ...
    formz_gentech_methods = models.ManyToManyField(
        ...
        related_name="plasmid_gentech_method",
        blank=True,
    ...
    formz_ecoli_strains = models.ManyToManyField(
        ...
        related_name="plasmid_ecoli_strains",
        blank=False,
    )
    ...
    def clean(self):
        ...
        if self.map:
            ...
            if self.map.size > file_size_limit:
                errors.append(
                    ValidationError("Plasmid map too large. Size cannot exceed 2 MB.")
                )
